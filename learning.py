...
def save_discretized_dataframe(indx, df_model, model_type, perm_type, bins, dataset_name, path, class_var):
    ...
    permutations : matrix
    	...
    	original vector
    """    
    file_path = path + dataset_name + "/" + str(indx) + "/" + re.sub( r"\.\w+", "", dataset_name ) + "_" + model_type +"_INDX_" + str(indx) + "_" + perm_type +"_permutations.csv"
...
def generate_permutations( instance, labels_lst, feature_names, class_var, encoder, scaler, model, samples = 300, variance = 0.25):
    ...
    if encoder!= None:
        ...
        labelled_predictions = [labels_lst[predictions[indx]] for indx in range(0, len(predictions))]
...
#
def generate_BN_explanationsMB(instance, label_lst, feature_names, class_var, encoder, scaler, model, path, dataset_name, variance = 0.25, algorithm = "Hill Climbing", show_in_notebook = True  ):
    ...
    bn, infoBN, essencGraph = learnBN( path_to_permutations, algorithm = algorithm)
...
def generate_local_predictions( X, Y, model, scaler, encoder ):
    ...
    for indx in range(0, orig_vec.shape[0]):
        ...
        if encoder!= None:
            ...
            ground_truth_class = ground_truth[0]
            prediction = prediction_class[indx]
